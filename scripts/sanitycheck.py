...
def extract_heap_summary(output):  
    ...
    # print("DEBUG"   , in_use_solution, total_usage_solution)
    # Set the solution to the expected output format
    if in_use_solution and total_usage_solution:
        solution = f"In use at exit: {in_use_solution}, Total heap usage: {total_usage_solution}"
        solution2 = f"Total heap usage: {total_usage_solution}"
    else:
        solution = "No heap summary found in output"
        solution2 = "No heap summary found in output"
    # print("Solution", solution, solution2)  
...
def perform_memcheck_test(test_case):
    ...
    if "HEAP SUMMARY" in output:
        memcheck, heapcheck = extract_heap_summary(output)
        if test_case["type"] == "memcheck":
            output = memcheck
        elif test_case["type"] == "heapcheck":
            output = heapcheck  
        else:
            output = "No heap summary found in output"
        b_output = output.encode("utf-8")
